// One-time mechanical AST migration. Run with --write only on pre-i18n sources.
...
for (const file of ['src/main.ts', 'src/protocol.ts', 'src/biography.ts']) {
  const source = readFileSync(file, 'utf8');
  if (process.argv.includes('--write') && source.includes('from "./i18n"')) throw new Error('Sources are already migrated; edit src/locales directly.');
...
if (process.argv.includes('--write')) { mkdirSync('src/locales', { recursive: true }); writeFileSync('src/locales/zh.json', JSON.stringify(catalog, null, 2) + '\n'); }
