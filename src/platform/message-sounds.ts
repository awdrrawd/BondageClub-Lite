...
export class MessageSounds {
  enabled = { beep: false, whisper: false, mention: false };
  private context: AudioContext | null = null;
  private last = 0;
  constructor() {
    try { const value=JSON.parse(localStorage.getItem('bc-lite-sounds-v1') || '{}'); this.enabled={beep:value.beep===true,whisper:value.whisper===true,mention:value.mention===true}; } catch { /* optional storage */ }
  }
  async enable(kind: 'beep' | 'whisper' | 'mention', value: boolean): Promise<void> {
  ...
  async unlock(): Promise<void> {
    if (!this.enabled.beep && !this.enabled.whisper && !this.enabled.mention) return;
    if (!this.context) this.context=new window.AudioContext();
    if (this.context.state==='suspended') await this.context.resume();
  }
  async play(kind: 'beep' | 'whisper' | 'mention', preview=false): Promise<void> {
