...
**相容發送預設開啟，可在面板取消：**原始線上封包缺少 AssetFamily 時比照 BC CharacterLoadOnline 使用 Female3DCG。相容模式將未實作條件與不完整偏好資料改為警告；自身自動效果未模擬僅作提示，不影響兩種模式的使用資格；已知裝備封鎖、禁止偏好、房間限制及缺失角色外觀仍不放行。角色／房間更新時刷新面板，發送時再次檢查。取消相容模式採嚴格判定。Lite 不更改偏好，也不宣稱完整 BC 引擎。詳見 [目前篩選範圍與驗收](activity-search-recovery-tests.md)。
